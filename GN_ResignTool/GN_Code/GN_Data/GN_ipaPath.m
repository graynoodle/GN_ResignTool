...
@implementation GN_Get_ipa_Path
...
-(BOOL)prepareForDragOperation:(id<NSDraggingInfo>)sender{
    ...
    [GN_Data setDataByKey:gn_IsIpa andValue:gn_YES];

    if ([path rangeOfString:@".ipa"].location != NSNotFound ||
        [path rangeOfString:@".zip"].location != NSNotFound) {
        path = [GN_Data getShellPath:path];
//        path = [path stringByReplacingOccurrencesOfString:@" " withString:@"\\ "];
//        path = [path stringByReplacingOccurrencesOfString:@"(" withString:@"\\("];
//        path = [path stringByReplacingOccurrencesOfString:@")" withString:@"\\)"];
        
        [GN_Data shared].gnIpaPath = path;
        [GN_Data setDataByKey:gn_IpaPath andValue:path];
        [GN_Data addControlLogs:@"ipa load success！\n "];
        [GN_Data shared].isIpa = YES;

    } else {
        if ([[GN_Data shared].gnIpaPath rangeOfString:@".ipa"].location != NSNotFound) {
            [GN_Data shared].isIpa = YES;
        } else {
            [GN_Data shared].isIpa = NO;
            [GN_Data setDataByKey:gn_IsIpa andValue:gn_NO];
        }
        [GN_Data alertSheetFirstBtnTitle:@"Ok" SecondBtnTitle:@"" MessageText:@"Hint" InformativeText:@"please load a correct IPA or zip"];

        [GN_Data addControlLogs:@"please load a correct IPA or zip\n "];
    }
